...
function PhotoUpload() {
  ...
  const [createPhoto, { mutationData, mutationLoading, mutationError }] =
    useMutation(CREATE_PHOTO);
  ...
  return (
    <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center' }}>
      {headshotUrl ? (
        <Image style={styles.image} source={{ uri: headshotUrl }} />
      ) : (
        <Image style={styles.image} source={MissingPerson} />
      )}
